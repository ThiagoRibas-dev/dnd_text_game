...
│   ├── D&D 3.5e Effects and State Specification.txt  # Detailed rules for effects and game state.
│   ├── D&D 3.5e Text RPG Game Spec.txt             # Overall game design and mechanics.
│   └── schemas/          # JSON/YAML schemas for content validation.
│       ├── CampaignDefinition.yaml # Schema for defining game campaigns.
│       └── StartingKit.yaml        # Schema for initial game setup/starting kits.
...
│   │   │   │   ├── divine_power.json # Example effect: Divine Power spell.
│   │   │   │   └── grease_area.json  # Example effect: Grease spell area.
│   │   │   ├── items/      # Definitions for all in-game items.
│   │   │   │   ├── armors/   # Definitions for armor items.
│   │   │   │   │   └── chain_shirt.yaml # Example armor: Chain Shirt.
│   │   │   │   ├── misc/     # Definitions for miscellaneous items.
│   │   │   │   │   ├── holy_symbol.yaml # Example item: Holy Symbol.
│   │   │   │   │   └── rations_5.yaml   # Example item: Rations.
│   │   │   │   ├── shields/  # Definitions for shield items.
│   │   │   │   │   └── heavy_wooden_shield.yaml # Example shield: Heavy Wooden Shield.
│   │   │   │   └── weapons/  # Definitions for weapon items.
│   │   │   │       └── heavy_mace.yaml # Example weapon: Heavy Mace.
...
│   └── dndrpg.egg-info/  # Python package metadata (generated by setuptools).
├── tests/                # Unit and integration tests for the game.
│   └── test_smoke.py     # Basic smoke tests to ensure core functionality.
...
├── run-codium.bat        # Batch script to run Codium (IDE).
├── start-game.bat        # Batch script to start the game.
